...
class FavouritePropertyView(APIView, CustomResponseMixin):
    ...
    def post(self, request, property_id):
        ...
        try:
            ...
            return self.custom_response(
                message="Invalid property ID", status=status.HTTP_400_BAD_REQUEST
            )
        ...
        Favourite.objects.create(
            user=user, property=property_obj, assigned_agent=property_obj.assigned_agent
        )
        return self.custom_response(
    ...
    def delete(self, request, property_id):
        ...
        if favourite.exists():
            ...
            return self.custom_response(
                message="Like removed successfully", status=status.HTTP_204_NO_CONTENT
            )
        else:
            return self.custom_response(
                message="Property was not liked.", status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def get(self, request):
        ...
        properties = Property.objects.filter(
            id__in=[f.property.id for f in favourite_properties]
        )
